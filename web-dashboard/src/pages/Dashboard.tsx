...
export const Dashboard: React.FC = () => {
  ...
  const steps = latestActivityUpdate?.steps !== undefined
    ...
    : (todaySteps?.steps ?? activitySummary?.totalSteps ?? 0);

  const goal = user?.dailyStepGoal || todaySteps?.goal || 10000;
  const progressPercent = Math.min(Math.round((steps / goal) * 100), 100);

  // Real-time metrics from WebSocket, todaySteps entity, or backend summary
  const rawDistance = latestActivityUpdate?.distanceMeters !== undefined && latestActivityUpdate.distanceMeters > 0
    ? latestActivityUpdate.distanceMeters
    : todaySteps?.distanceMeters && todaySteps.distanceMeters > 0
    ? todaySteps.distanceMeters
    : activitySummary?.totalDistanceMeters && activitySummary.totalDistanceMeters > 0
    ? activitySummary.totalDistanceMeters
    : steps * 0.753;
  const distanceKm = (rawDistance / 1000).toFixed(2);

  const rawCalories = latestActivityUpdate?.caloriesBurned !== undefined && latestActivityUpdate.caloriesBurned > 0
    ? latestActivityUpdate.caloriesBurned
    : todaySteps?.caloriesBurned && todaySteps.caloriesBurned > 0
    ...
    : steps * 0.04;
  const caloriesBurned = Math.round(rawCalories);
